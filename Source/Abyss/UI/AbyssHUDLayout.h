...
public:
  ...
  // 要绑定的退出键
  UPROPERTY(EditDefaultsOnly, Category = "UI Options Screen",
...
protected:
  ...
  /**
   * Callback for when controllers are disconnected. This will check if the
   * player now has no mapped input devices to them, which would mean that they
   * can't play the game.
   *
  ...
  /**
   * Callback for when controllers change their owning platform user. We will
   * use this to check if we no longer need to display the "Controller
   * Disconnected" menu
   */
  /**
   ...
   *
   */
  void HandleInputDevicePairingChanged(FInputDeviceId InputDeviceId,
                                       FPlatformUserId NewUserPlatformId,
                                       FPlatformUserId OldUserPlatformId);

  /**
   * Notify this widget that the state of controllers for the player have
   * changed. Queue a timer for next tick to process them and see if we need to
   * show/hide the "controller disconnected" widget.
   */
  /**
  ...
  /**
   * This will check the state of the connected controllers to the player. If
   * they do not have any controllers connected to them, then we should display
   * the Disconnect menu. If they do have controllers connected to them, then we
   * can hide the disconnect menu if its showing.
   */
  /**
  ...
  /**
   * Pushes the ControllerDisconnectedMenuClass to the Menu layer
   * (UI.Layer.Menu)
   */
  /**
...
protected:
  /**
   * The menu to be displayed when the user presses the "Pause" or "Escape"
   * button 当用户按下“暂停”或“退出”按钮时将显示的菜单内容
   */
  UPROPERTY(EditDefaultsOnly)
  TSoftClassPtr<UCommonActivatableWidget> EscapeMenuClass;

  /**
   * The widget which should be presented to the user if all of their
   * controllers are disconnected.
   * 当用户的所有控制器均处于断开状态时，应向用户展示的这个小部件。
   */
  // UPROPERTY(EditDefaultsOnly, Category="Controller Disconnect Menu")
  // TSubclassOf<ULyraControllerDisconnectedScreen>
  // ControllerDisconnectedScreen;

  /**
   * The platform tags that are required in order to show the "Controller
   * Disconnected" screen.
   *
   * If these tags are not set in the INI file for this platform, then the
   * controller disconnect screen will not ever be displayed.
   */
  /**
   * 显示“控制器断开连接”界面所需的平台标签。
   * 如果在该平台的 INI
   * 文件中未设置这些标签，那么控制台断开连接的屏幕将永远不会显示出来。
   *
   */
  UPROPERTY(EditDefaultsOnly, Category = "Controller Disconnect Menu")
  ...
  /** Handle from the FSTicker for when we want to process the controller state
   * of our player */
...
protected:
  ...
  TObjectPtr<class UImage> PrimaryWeaponIcon;

  UPROPERTY(meta = (BindWidget))
  TObjectPtr<class UImage> SecondaryWeaponIcon;

  UPROPERTY(meta = (BindWidget))
  TObjectPtr<class UImage> MeleeWeaponIcon;

  UPROPERTY(meta = (BindWidget))
  TObjectPtr<class UCommonTextBlock> CurrentClipAmmo;

  UPROPERTY(meta = (BindWidget))
  TObjectPtr<class UCommonTextBlock> ReserveAmmo;
  ...
  // Helper
  class AAbyssPawn *GetAbyssPawn() const;
  class UAbyssInventoryComponent *GetInventoryComponent() const;
};
