...
void UAbyssInteractionComponent::TickComponent(
  ...
  if (GetOwningPawn()->IsLocallyControlled() &&
    ...
    if (UAbyssInputComponent *AbyssEnhancedInputComponent =
      ...
      AbyssEnhancedInputComponent->BindAction(
          ...
          &ThisClass::Input_Interact_Started);

      // 绑定 "松开" (Completed) -> 用于处理长按中断
      AbyssEnhancedInputComponent->BindAction(
          InteractAction, ETriggerEvent::Completed, this,
          &ThisClass::Input_Interact_Completed);

      // 修复 Bug：如果是瞬时按下并松开，有时候会触发 Canceled 而不是 Completed
      AbyssEnhancedInputComponent->BindAction(
          InteractAction, ETriggerEvent::Canceled, this,
          &ThisClass::Input_Interact_Completed);
    }
  }
}

void UAbyssInteractionComponent::Input_Interact_Started(
...
void UAbyssInteractionComponent::Input_Interact_Completed(
    const FInputActionValue &Value) {
  if (ActivePickupWidget.IsValid()) {
...
void UAbyssInteractionComponent::PerformInteractionCheck() {
  ...
  // 将屏幕坐标反投影到世界方向
  PC->DeprojectScreenPositionToWorld(ScreenX, ScreenY, ViewLocation,
                                     ViewDirection);
  ...
  bool bHit = GetWorld()->LineTraceSingleByChannel(HitResult, Start, End,
                                                   ECC_Interaction, Params);

  // bool bHit
  // =UKismetSystemLibrary::LineTraceSingle(GetWorld(),Start,End,ETraceTypeQuery::TraceTypeQuery3,false,TArray<AActor*>(),
  ...
  if (bHit && HitResult.GetActor() &&
    ...
    if (HitResult.GetActor() != CurrentInteractable.GetObject()) {
      ...
      CurrentInteractable.SetInterface(
          Cast<IInteractableInterface>(HitResult.GetActor()));

      // 通知 UI
      FInteractionData Data =
          IInteractableInterface::Execute_GetInteractionData(
              CurrentInteractable.GetObject());
...
void UAbyssInteractionComponent::OnFoundInteractable(
  ...
  if (LocalPlayer) {
    ...
    ActivePickupWidget = Cast<UPickupPromptWidget>(
        UCommonUIExtensions::PushContentToLayer_ForPlayer(
            LocalPlayer, AbyssTags_UI::Layer::Modal, PickupWidget));
    ActivePickupWidget->SetupInteraction(Data);
    ActivePickupWidget->OnLongPressComplete.AddDynamic(this,
                                                       &ThisClass::TryInteract);
    ActivePickupWidget->OnShortPressTriggered.AddDynamic(
        this, &ThisClass::TryInteract);
  }
}
...
void UAbyssInteractionComponent::ServerTriggerInteraction_Implementation(
    UObject *TargetObject) {
  if (TargetObject && TargetObject->Implements<UInteractableInterface>()) {
    IInteractableInterface::Execute_TriggerInteraction(TargetObject,
                                                       Cast<APawn>(GetOwner()));
  }
}
