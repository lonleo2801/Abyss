...
void UAbyssProjectileAbility::ActivateAbility(
...
void UAbyssProjectileAbility::SpawnProjectile(FVector ProjectileTargetLocation,
  ...
  if (CombatInterface) {
    ...
    // CombatInterface->GetCombatSocketLocation(SocketTag);
    const FVector SocketLocation =
        GetAvatarActorFromActorInfo()->GetActorLocation();

    FRotator Rotation;
    if (ProjectileTargetLocation.IsZero()) {
      // 【关键修复】使用缓存的旋转，而不是实时查询
      // 这样服务器会使用客户端发送过来的预测旋转，解决方向不一致问题
      // 需要在子类中保存 CachedFireRotation
