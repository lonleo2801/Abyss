...
AAbyssWeaponBase::AAbyssWeaponBase()
{
    CurrentState = EWeaponState::OnGround;
}

void AAbyssWeaponBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
...
void AAbyssWeaponBase::UpdateAttachmentLogic()
{
    ...
    if (!WeaponDef)
    {
        UE_LOG(LogTemp, Warning, TEXT("Weapon %s matches no WeaponDefinition!"), *GetName());
        return;
    }
    ...
    USkeletalMeshComponent* TargetMesh = OwnerActor->FindComponentByClass<USkeletalMeshComponent>();
    ...
    if (TargetMesh)
    {
        ...
        {
            AttachToComponent(TargetMesh, FAttachmentTransformRules::SnapToTargetNotIncludingScale, TargetSocket);
        }
    }
    else
    {
        UE_LOG(LogTemp, Warning, TEXT("Owner %s has no SkeletalMesh to attach weapon!"), *OwnerActor->GetName());
    }
}
...
bool AAbyssWeaponBase::TriggerInteraction_Implementation(APawn* Interactor)
{
    ...
    if (CurrentState != EWeaponState::OnGround) return false;

    // 找到交互者的背包组件
    if (UAbyssInventoryComponent* Inventory = Interactor->FindComponentByClass<UAbyssInventoryComponent>())
    {
        // 发送拾取请求
